 ...
 public class PseudoThread extends EventDispatcher
 {
	...
	private function pseudoDone():void
	{
	 ...
	 private function renderHandler(event:Event):void
	 {
		 ...
		 {
			 pseudoDone();
			 trace("PseudoThread:renderHandler - user canceled");
			 return;
		 }
		 if (mouseEvent || keyEvent)
			 due -= RENDER_DEDUCTION;

		 while (!_userCanceled && (getTimer() < due))
		 {
			try
			{
				res = fn.apply(threadThis, obj);
			}
			catch(e:Error)
			{
				trace("renderHandle:exception thrown: " + e.message);
				res = false;
			}
		 ...
		 if(_userCanceled)
		 {
			trace("PseudoThread:renderHandler - user canceled caused  drop  out of loop");
		 }
		 mouseEvent = false;
		 keyEvent = false;
	 }
