...
package com.hurlant.crypto.tls {
	...
	public class TLSSocket extends EventDispatcher implements IDataInput, IDataOutput {
		...
		// AM - propagate ignore host identity
		private var _ignoreHostIdentity:Boolean;
		...
		public function close():void {
			...
			{
				trace("Error closing SSL socket : "+e.message);
			}
		}
		...
		private function init(socket:Socket, config:TLSConfig, host:String):void {
			...
			//AM ignore host identity if caller wants so
			_engine = new TLSEngine(config, _socket, _socket, (_ignoreHostIdentity ? null : host));
			...
			_engine.addEventListener(ProgressEvent.SOCKET_DATA, function(e:*):void {if(_socket.connected){ _socket.flush();} });
		...
		public function get ignoreHostIdentity():Boolean
		{
			return _ignoreHostIdentity;
		}

		public function set ignoreHostIdentity(value:Boolean):void
		{
			_ignoreHostIdentity = value;
		}
