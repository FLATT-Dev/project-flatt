...
package com.hurlant.crypto.tls {
	...
	public class TLSEngine extends EventDispatcher {
		...
		function TLSEngine(config:TLSConfig, iStream:IDataInput, oStream:IDataOutput, otherIdentity:String = null) {
			...
			_iStream = iStream;
			_oStream = oStream;
			// TODO: implemennt wildcard support
			//AM - otherIdentity can be null, cert identity (i.e host that issued it )is not checked
		...
		//----------------------------------------------
		/* XXX!!
		From SSL implementation  sun.security.ssl.Record file
		...
		*/
		public function sendApplicationData(data:ByteArray/*, offset:uint=0, length:uint=0*/):void 
		{
			var rec:ByteArray = new ByteArray;
			//var len:uint = length;
			// payload can be no larger than 16384 or 2^14 bytes of data
			// if we have more- chunk it
			if(data.length >= 16384)
			{
				trace("**TROUBLE** TLS Engine:Data len = " + data.length + "!!!");
			}
			/* There is a 16K limitation on  the size of data that can be sent via SSL
				If our data is > max data size chunk it into 8K blocks and send 
			*/
			if(data.length >=16384)
			{
				//var len:int =0;
				...
				{
					curChunk = Math.min(data.length-offset,8192);
					rec.clear();									
					rec.writeBytes(data, offset, curChunk);
					rec.position = 0;
					trace("TLS sendApplicationData:cur chunk = " +curChunk);
